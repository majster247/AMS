...
void sound(char* args, CommandLine* cli){
...
void dbeep(uint32_t arg1, uint32_t arg2, CommandLine* cli) {
	
    if (arg1 < 1 || arg2 <= 0) {
        printf("Invalid frequency or duration.\n");
        return;
    }

    // Calculate the total time for the beeping
    uint32_t totalTime = arg1 * arg2;

    for (int i = 0; i < totalTime; i += arg1) {
        makeBeep(arg1);  // Beep for 'freq' milliseconds
        sleep(arg1);  // Pause for 'freq' milliseconds before the next beep
    }
}
...
void version(char* args, CommandLine* cli) {
	...
	printf("Developed by: Hubert 'Majster' Topolski\n");
	printf("Major update no. 20, pre-alpha release\n");
...
void CommandLine::script(bool pressed, char key, bool ctrl) {
	...
	//interpret line
	ScriptInput(this, file, fileSize, indexF, nestedLoop, indexLoopF);
...
char* CommandLine::command(char* cmd, uint8_t length) {
	...
	if (this->conditionIf == false && result != hash("fi")) {	
		return "CIA watching, idc what you writing";
	}
