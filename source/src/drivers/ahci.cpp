...
bool ahci_send_command(ahci_port* port, uint8_t cmd, uint64_t lba, uint32_t count, uint16_t* buffer) {
    port->is = 0xFFFFFFFF; // Czyść przerwania
    int slot = 0;

    // Alokacja bufora DMA (musi być fizyczny!)
    ...
    // Czekaj na wolny dysk
    while (port->tfd & (0x80 | 0x08));

    // BARIERA: Zmuś CPU do wysłania danych do RAM przed startem DMA
    asm volatile("wbinvd" ::: "memory");
    asm volatile("mfence" ::: "memory");

    port->ci = (1 << slot); // START

    // Pętla oczekiwania
    while (true) {
        if (!(port->ci & (1 << slot))) break; // Sukces - bit zgasł

        if (port->is & (1 << 30)) { // TFES
            // SPRAWDZENIE: Jeśli TFD.STS.ERR (bit 0) nie jest ustawiony, ignorujemy błąd widmo
            if (!(port->tfd & 0x01)) {
                if (!(port->ci & (1 << slot))) break;
                continue; 
            }
            write_serial_string("[AHCI] Real Hardware Error!\n");
