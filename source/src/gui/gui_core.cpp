        ...
        if (mx >= lx && mx <= lx + menu_w && my >= ly && my <= ly + menu_h) {
            ...
            if (rel_y >= 60) {
                ...
                switch(item) {
                    ...
                    case 6: {
                        char* wl_argv[2];
                        wl_argv[0] = (char*)"/programs/wayland/wayland-session";
                        wl_argv[1] = nullptr;
                        launch_user_app("/programs/wayland/wayland-session", 1, wl_argv);
                        break;
                    }
                    case 7: {
                        char* wlc_argv[2];
...
void Desktop::HandleKeyboard(char c) {
    ...
    // Szukamy ostatniego nie-nulla (okno na wierzchu)
    for (int i = 31; i >= 0; i--) {
...
void Desktop::DrawClock(uint32_t x, uint32_t y) {
    int h, m, s;
    get_time(h, m, s);
    char time_str[9];
    // ... formatowanie ...
    ...
    graphics_print(x, y, time_str, 0xFFFFFF);
}